			...
			this.google.api.client.calendar.calendars
				...
				.then((res) => {
					...
					this.user.userSchedule.forEach((a) => {
						...
						let end = dayjs(a.day, 'DD/MM/YYYY').format('YYYY-MM-DD') + 'T' + this.convertLessonsToTime(a.lesson).end + ':00.000+07:00';
						let event = {
		...
		async ImportMicrosoftCalendar() {
			...
			try {
				...
				this.user.userSchedule.forEach(async (a, index) => {
					setTimeout(async () => {
						let start = dayjs(a.day, 'DD/MM/YYYY').format('YYYY-MM-DD') + 'T' + this.convertLessonsToTime(a.lesson).start + ':00';
						let end = dayjs(a.day, 'DD/MM/YYYY').format('YYYY-MM-DD') + 'T' + this.convertLessonsToTime(a.lesson).end + ':00';
						let event = {
